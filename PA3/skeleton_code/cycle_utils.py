...
def save_samples(iteration, fixed_Y, fixed_X, G_YtoX, G_XtoY, opts):
    ...
    merged = ((merged + 1.0) / 2.0 * 255.0).clip(0, 255).astype(np.uint8)
    path = os.path.join(opts.sample_dir, f'sample-{iteration:06d}-X-Y.png')
    ...
    merged = ((merged + 1.0) / 2.0 * 255.0).clip(0, 255).astype(np.uint8)
    path = os.path.join(opts.sample_dir, f'sample-{iteration:06d}-Y-X.png')
    imageio.imwrite(path, merged)
    print(f'Saved {path}')
